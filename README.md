...
### 🔐 Added: End‑to‑end encryption (ECIES)
...
- The script will automatically:  
  - Encrypt the message using ECDH (ephemeral key) + HKDF + AES‑256‑GCM  
  - Transmit the ciphertext as an opaque base64 blob (no prefix, no marker)  
...
4. Nodes that retransmit reward whoever retransmitted before them  
...
# (paste the entire GhostRelay script here)
EOF
```
...
addcontact <name> <key> Add a contact for encrypted messaging
...
addnode <name> <key> Add a trusted relay node
mykey Show your public key to share with others
clear Clear the screen
<text>:<contact> Send an encrypted message to that contact
...
Done. Your device is now part of the GhostRelay network, retransmitting messages, earning credits based on the order of return, and able to send end‑to‑end encrypted messages to any contact whose public key you have added.
