...
def clean_caches_once():
    ...
    with cache_lock:
        ...
        known_originals = set(original_cache)
    # credited_order tracks only messages WE created → clean by original_cache
    with credited_order_lock:
        for p in [p for p in credited_order if p not in known_originals]:
            del credited_order[p]
    # _relay_state expires by its own TTL (60 s), not by cache membership
...
# Priority queue
#
# Heap entry:  (-value, enqueue_time, payload, origin_fp, size)
#   value     — current relay value for this specific message; starts at
#               get_credit(origin_fp) at first enqueue, then decays with each
#               successful retransmission: value = floor(base / (n_tx + 1))
#   -value    — negated so heapq (min-heap) pops the highest-value item first
#
# Per-payload relay state (protected by queue_lock):
#   _relay_state[payload] = {
#       "base":   int,    # value at first enqueue (never changes)
#       "n_tx":   int,    # number of successful retransmissions so far
#       "born":   float,  # time.monotonic() when first enqueued (for TTL)
#   }
#
# The credit system (credits.json / add_credit) is completely untouched.
# ------------------------------------------------------------------------------
...
def retransmit_worker(transport: Transport, stop_event: threading.Event):
    ...
    # Minimum wall-clock gap between two consecutive retransmissions of the
    # *same* payload by this node.  Prevents a single message from monopolising
    # the radio when it is the only item in the queue.
    MIN_RETX_INTERVAL = 0.5   # seconds

    # last_sent[payload] = time.monotonic() of the most recent successful send
    last_sent: dict = {}
    ...
    while not stop_event.is_set():
        ...
        # If we sent this payload too recently, put it back and wait a bit.
        now = time.monotonic()
        ...
        try:
            ...
            mark_relayed(payload, MY_NODE_ID)   # records (payload, MY_NODE_ID) for 20 min
            last_sent[payload] = time.monotonic()
        except Exception as e:
            log.warning("[TX] Send failed, re-queuing: %s", e)
            with queue_lock:
                heapq.heappush(priority_queue, item)   # re-push unchanged
            time.sleep(0.5)
            continue

        # ── Success: compute decayed value, then increment n_tx ───────────────
        #
        # IMPORTANT order of operations:
        #   1. Read n_tx (retransmissions already completed before this one).
        #   2. Compute new_value = base // (n_tx + 1)   ← divisor starts at 1
        #   3. Increment n_tx.
        #
        # This gives the sequence:  base//1, base//2, base//3, …
        # Incrementing BEFORE computing would skip the full-value first step.
        with queue_lock:
            state = _relay_state.get(payload)
            if state is not None:
                n_tx_before  = state["n_tx"]                      # step 1
                new_value    = state["base"] // (n_tx_before + 1) # step 2
                state["n_tx"] += 1                                 # step 3
            else:
                n_tx_before = 0
                new_value   = 0

            # Deduct the size of the item we popped from the buffer counter.
            current_buffer_bytes = max(0, current_buffer_bytes - size)
...
def handle_lora_packet(content: str):
    ...
    2. Timestamp validity  (rejects stale / future packets early).
    3. Signature identification  (discard unknown signers).
    4. Discard our own echoes.
    5. Return-to-author check  (payload in original_cache → credit, no dedup gate).
    6. Session dedup via seen_packets  (avoids re-processing the same packet).
    7. Anti-loop via relayed_cache  ((payload, signer_fp) already relayed → skip).
    8. Display + enqueue for relay.
    """
    ...
    # ── Step 2: timestamp first — cheapest filter, no crypto needed ───────────
    if not is_timestamp_valid(payload[-TIMESTAMP_CHARS:]):
        log.debug("[RX] Timestamp out of window")
        return

    # ── Step 3: identify signer ───────────────────────────────────────────────
    ...
    # ── Step 4: ignore our own echoes ─────────────────────────────────────────
    if signer_fp == MY_NODE_ID:
        return

    # ── Step 5: return-to-author → grant credit ───────────────────────────────
    # Credit is given ONLY when WE are the original author (payload in
    # original_cache).  relayed_cache does not trigger credit.
    # We do NOT block on already_seen here: the same relayer may legitimately
    # deliver the same payload back to us multiple times (different positions),
    # each earning a smaller reward.
    if payload in original_cache:
        with credited_order_lock:
            ...
            order.append(signer_fp)             # duplicates allowed
        add_credit(signer_fp, reward)
        log.info("[CREDIT] %s | place %d | +%d pts", signer_fp, place, reward)
        return

    # ── Step 6: session dedup — only for forwarding path ─────────────────────
    if already_seen(payload, signer_fp):
        return
    mark_seen(payload, signer_fp)

    # ── Step 7: anti-loop — skip if signer already relayed this payload ───────
    # relayed_cache key is (payload, node_id); a different signer carrying the
    # same payload is a distinct relay event and should still be forwarded.
    if has_relayed(payload, signer_fp):
        log.debug("[RX] Already relayed by %s — skipping", signer_fp)
        return

    # ── Step 8: display and enqueue ───────────────────────────────────────────
...
# ------------------------------------------------------------------------------
# stdin reader thread  (cross-platform replacement for select on stdin)
#
# select.select([sys.stdin], ...) raises WinError 10038 on Windows because
# Windows only supports select() on real sockets, not on file descriptors.
# The fix: a dedicated daemon thread that calls sys.stdin.readline() (which
# blocks cleanly on all platforms) and pushes each stripped line into a
# queue.Queue.  The main loop drains that queue with get_nowait() — the same
# pattern already used for the transport line_queue — so no platform-specific
# code is needed anywhere else.
# ------------------------------------------------------------------------------

def stdin_reader_thread(stdin_queue: queue.Queue):
    """
    Blocking readline loop for sys.stdin.
    Puts each non-empty stripped line into stdin_queue.
    Puts the sentinel None on EOF so the main loop can react.
    Safe on Windows, Linux, macOS, and Termux/Android.
    """
    try:
        while True:
            try:
...
""")
    ...
    # Cross-platform stdin reader — works on Windows, Linux, macOS, Termux.
    # A single thread is started here (not inside the reconnect loop) so that
    # user input is never lost during transport reconnections.
    ...
    while True:
        ...
        try:
            ...
            while not connection_lost:
                ...
                # ── stdin — drain commands queued by stdin_reader_thread ──
                # get_nowait() never blocks; works on Windows, Linux, macOS.
                # A small sleep keeps CPU usage negligible when both queues
                # are empty (transport idle + no user input).
                try:
                ...
                if cmd is None:   # sentinel: stdin closed (EOF / Ctrl-D)
                    break
