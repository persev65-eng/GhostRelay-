...
def encrypt_for_contact(plaintext: str, vk_crypto) -> str:
    ...
    The output is indistinguishable from random data to any third party.
...
def decrypt_message(content: str):
    ...
    the AES-GCM authentication tag will reject them cleanly.
    """
    try:
        # validate=True rejects strings with non-base64 characters immediately
...
def retransmit_worker(sock, stop_event):
    ...
    while not stop_event.is_set():
        ...
        # when the entire queue has stale priorities.
