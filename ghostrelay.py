...
def clean_caches_once():
    ...
    # Purge relay-state entries only by their own TTL (RELAY_TTL_SEC = 60 s).
    # Do NOT remove them based on relayed_cache / original_cache expiry (those
    # use CACHE_EXPIRE_SEC = 20 min): a message can finish all its relay rounds
    # long before those caches expire, and a message that left relayed_cache
    # must still be re-transmittable until its own 60 s TTL is up.
...
def enqueue(payload: str, origin_fp: str):
    ...
    The initial value equals the sender's current credit; subsequent re-enqueues
    (after retransmission) are handled internally by retransmit_worker.
    """
    ...
    with queue_lock:
        ...
        if payload not in _relay_state:
            base = max(1, get_credit(origin_fp))   # snapshot — won't change
            _relay_state[payload] = {
                "base": base,
                "n_tx": 0,
                "born": time.monotonic(),
...
def retransmit_worker(transport: Transport, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        # ── Success: update state and re-enqueue with decayed value ──────────
        with queue_lock:
            # state may have been cleaned up by cache_cleaner between the
            # TTL check above and now — guard defensively.
            state = _relay_state.get(payload)
            if state is not None:
                state["n_tx"] += 1
                new_value = state["base"] // (state["n_tx"] + 1)
            else:
                new_value = 0

            # Always deduct size for the item we just popped.
            current_buffer_bytes = max(0, current_buffer_bytes - size)

            # Re-enqueue only if the decayed value is still useful and the
            # payload hasn't expired (TTL checked again inside the lock).
            ...
            if (
                ...
                if current_buffer_bytes + size <= MAX_BUFFER_BYTES:
                    ...
                    current_buffer_bytes += size
                    log.info(
                        "[TX] Relayed+requeued | origin=%s n_tx=%d value=%d→%d",
                        origin_fp, state["n_tx"], -neg_value, new_value,
                    )
                else:
                    log.warning("[TX] Buffer full — not requeuing after relay")
                    _relay_state.pop(payload, None)
            else:
                # Value hit 0 or TTL expired — clean up state.
                _relay_state.pop(payload, None)
                log.info(
                    "[TX] Relayed (final) | origin=%s n_tx=%d value_was=%d",
                    origin_fp,
