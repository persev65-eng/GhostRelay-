...
"""
        ...
        # race.py guarda hash e pontos, mas não guarda com que SF/BW/CR
        # a mensagem saiu, e a seção 21 exige essa comparação na volta.
        # TODO: quando race.py for corrigido, isto sai daqui.
        self.corrida_radio = {}
...
"""
    ...
    def registrar_corrida(self, msg_hash, pontos, radio):

        if not self.race:

            return

        if self.race.find(msg_hash):

            return

        self.race.add_message(msg_hash, pontos)

        # guarda com que rádio a mensagem saiu (seção 21)
        self.corrida_radio[msg_hash] = dict(radio)

        # o dicionário auxiliar não pode crescer mais que a corrida
        if len(self.corrida_radio) > self.race.max_size:

            for chave in list(self.corrida_radio.keys()):

                if not self.race.find(chave):

                    del self.corrida_radio[chave]
    ...
    def processar_pacote_recebido(self, event):
        ...
        if CORRIDA_RETRANSMITIDAS:
            ...
            self.registrar_corrida(msg_hash, pontos, radio)
        ...
        #
        # Só o texto: o enviar_para_aplicacao() do server.py atual já
        # embrulha em {"tipo":"rx","dados":...}. Mandar um dicionário
        # aqui viraria JSON dentro de JSON na tela.
        # TODO: quando server.py for corrigido, mandar também vizinho,
        # rssi, snr e SF/BW/CR.
        self.entregar_para_app(conteudo)
    ...
    def recompensar(self, msg_hash, conteudo, assinatura, radio):
        ...
        # Seção 21: só paga se voltou com o MESMO SF/BW/CR.
        original = self.corrida_radio.get(msg_hash)

        if original and not self.economy.reward_allowed(original, radio):

            print("SEM RECOMPENSA para %s: SF/BW/CR diferentes do original"
                  % dono[:12])

            return

        # Seção 10: a recompensa cai pela metade a cada volta
        pontos = self.race.reward_relay(msg_hash, dono)
        ...
        print("RECOMPENSA: +%.0f pontos para %s (total %.0f)"
              % (pontos, dono[:12], resultado["total"]))
