...
invite/candidates, race ranking, independent expunge windows, and local API server.

Connect external apps to 127.0.0.1:9999 and send/receive messages.
"""
...
# ----------------------------------------------------------------------------
# Local API Server (for external apps)
# ----------------------------------------------------------------------------

connected_clients = set()
clients_lock = threading.Lock()
external_cmd_queue = queue.Queue()  # Tuples ("send", text) or ("cmd", command_str)

def broadcast_to_apps(msg: str):
    """Send a message to all connected local API clients."""
    with clients_lock:
        dead = set()
...
def local_api_server():
    ...
    server.bind((LOCAL_API_HOST, LOCAL_API_PORT))
    server.listen(5)
    log.info("[LOCAL API] Listening on %s:%d", LOCAL_API_HOST, LOCAL_API_PORT)
    ...
    def accept_client(sock):
        ...
        log.info("[LOCAL API] App connected from %s", addr)
            ...
            if data:
                ...
                for line in lines:
                    ...
                    if lower.startswith("send "):
                        text = line[5:].strip()
                        external_cmd_queue.put(("send", text))
                    else:
                        external_cmd_queue.put(("cmd", line))
            else:
                log.info("[LOCAL API] App disconnected from %s", addr)
                sel.unregister(conn)
                conn.close()
                with clients_lock:
                    connected_clients.discard(conn)
        except Exception as e:
            log.error("[LOCAL API] Error handling client %s: %s", addr, e)
            sel.unregister(conn)
            conn.close()
            with clients_lock:
                connected_clients.discard(conn)
...
def handle_lora_packet(content: str) -> bool:
    ...
    if decrypted is not None:
        msg = f"🔒 [PRIVATE] de {signer_fp}: {decrypted}"
        print(f"\n{msg}\n> ", end="", flush=True)
        broadcast_to_apps(f"RECV PRIVATE {signer_fp}: {decrypted}")
    else:
        msg = f"📢 [OPEN]    de {signer_fp}: {msg_content}"
        print(f"\n{msg}\n> ", end="", flush=True)
        broadcast_to_apps(f"RECV OPEN {signer_fp}: {msg_content}")
...
def cmd_credits(file=None):
    output = file if file else sys.stdout
    with trusted_lock:
        snap = sorted(_trusted.items(), key=lambda x: x[1].points, reverse=True)
    print("\n=== TRUSTED (points) ===", file=output)
    if snap:
        for fp, entry in snap:
            print(f"  {fp} : {entry.points} pts", file=output)
    else:
        print("  (no trusted nodes yet)", file=output)
    print("========================\n", file=output)


def cmd_trusted(file=None):
    output = file if file else sys.stdout
    with trusted_lock:
        snap = sorted(_trusted.items(), key=lambda x: x[1].points, reverse=True)
    print("\n=== TRUSTED NODES (points) ===", file=output)
    if snap:
        for fp, entry in snap:
            print(f"  {fp} : {entry.points} pts", file=output)
    else:
        print("  (none)", file=output)
    print(f"  MY ID : {MY_NODE_ID}", file=output)
    print("=============================\n", file=output)
...
def cmd_queue(file=None):
    ...
    if not snap:
        print("\n=== QUEUE empty ===\n", file=output)
        return
    print(f"\n=== QUEUE ({len(snap)} msgs · {total} bytes) ===", file=output)
...
def cmd_wifi_scan(file=None):
    import shutil
    import subprocess

    output = file if file else sys.stdout
    print("\n=== WIFI SCAN ===", file=output)
    ...
    if result:
        ...
        print("  No supported WiFi scan tool found.", file=output)

    ap_ip = CFG.get("wifi_ap_ip", "192.168.4.1")
    ssid = CFG.get("wifi_ap_ssid", "")
    print(f"\n  Configured ESP32 AP  : {ssid or '(wifi_ap_ssid not set in config.json)'}", file=output)
    print(f"  Configured ESP32 IP  : {ap_ip}", file=output)
    print("=================\n", file=output)
...
""")
    ...
    if LOCAL_API_ENABLED:
        print(f"[START] Local API       : {LOCAL_API_HOST}:{LOCAL_API_PORT}")
    print()
    ...
    if LOCAL_API_ENABLED:
        print("Local API commands (for connected apps):")
        print("  SEND <message>           — send plaintext message")
        print("  SEND <msg>:<contact>     — send encrypted message")
        print("  CREDITS, QUEUE, etc.    — run command and get output")
    print()

    threading.Thread(target=cleanup_thread, daemon=True, name="cleanup").start()
    if LOCAL_API_ENABLED:
        threading.Thread(target=local_api_server, daemon=True, name="local-api").start()
    ...
    while True:
        ...
        try:
            ...
            while not connection_lost:
                ...
                # Check for stdin (keyboard)
                stdin_cmd = None
                try:
                    stdin_cmd = stdin_queue.get_nowait()
                except queue.Empty:
                    pass
                ...
                ext_cmd = None
                try:
                    ext_cmd = external_cmd_queue.get_nowait()
                except queue.Empty:
                    pass

                if ext_cmd is not None:
                    cmd_type, payload = ext_cmd
                    if cmd_type == "send":
                        text = payload
                        contact_name = None
                        if ":" in text:
                            ...
                            contact = right.strip()
                            if contact and get_contact(contact):
                                text = left.strip()
                                contact_name = contact
                        ok = send_normal_message(transport, text, encrypt_to=contact_name)
                        if ok:
                            if contact_name:
                                broadcast_to_apps(f"SENT ENCRYPTED to '{contact_name}'")
                            else:
                                broadcast_to_apps("SENT PLAINTEXT")
                        else:
                            broadcast_to_apps("ERROR Message not sent")
                    else:  # "cmd"
                        cmd_str = payload
                        buf = StringIO()
                        low = cmd_str.lower()
                        if low == "credits":
                            cmd_credits(file=buf)
                        elif low == "queue":
                            cmd_queue(file=buf)
                        elif low == "trusted":
                            cmd_trusted(file=buf)
                        elif low == "contacts":
                            cmd_contacts(file=buf)
                        elif low == "mykey":
                            ...
                            clear_candidates()
                            broadcast_to_apps("CANDIDATES cleared")
                        elif low.startswith("addnode "):
                            parts = cmd_str.split(maxsplit=2)
                            if len(parts) == 3:
                                ok = add_trusted_node(parts[1], parts[2])
                                broadcast_to_apps("Node added" if ok else "Invalid key")
                            else:
                                broadcast_to_apps("Usage: addnode <name> <base64_key_or_PEM>")
                        elif low.startswith("addcontact "):
                            parts = cmd_str.split(maxsplit=2)
                            if len(parts) == 3:
                                ok = add_contact(parts[1], parts[2])
                                broadcast_to_apps("Contact added" if ok else "Invalid key")
                            else:
                                broadcast_to_apps("Usage: addcontact <name> <base64_key_or_PEM>")
                        elif low == "invite":
                            send_invite(transport)
                            broadcast_to_apps("INVITE sent")
                        elif low == "wifiscan":
                            cmd_wifi_scan(file=buf)
                        elif is_esp_cmd(cmd_str):
                            safe_send(transport, f"{cmd_str}\r\n".encode("utf-8"))
                            broadcast_to_apps(f"CMD {cmd_str}")
                        else:
                            # Unknown command, ignore or notify
                            pass

                        output = buf.getvalue()
                        if output:
                            broadcast_to_apps(output.rstrip())

                time.sleep(0.02)  # small sleep to prevent busy wait
